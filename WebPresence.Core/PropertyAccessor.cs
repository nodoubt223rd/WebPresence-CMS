    ...
    public class PropertyAccessor
    {
        ...
        protected DisplayNameAttribute Display
        {
            get
            {
                ...
                if (displays == null || displays.Length == 0)
                    ...
                    return displays[0] as DisplayNameAttribute;
            }
        }
        ...
        public string DisplayName
        {
            get
            {
                ...
                if (display == null || string.IsNullOrWhiteSpace(display.DisplayName))
        ...
        protected void Initializer(object destination, string expression, Type type)
        {
            ...
            if (!metaDataOnly && expression.Contains("["))
                throw new NotSupportedException(string.Format(CoreResources.NotSupportedEnumerables, Constants.PROPERTY_SETTER));
            ...
            foreach (string roughField in fields)
            {
                ...
                if (metaDataOnly && field.IndexOf('[') >= 0)
                ...
                if (currProperty == null)
                {
                    if (locked || index >= fields.Length - 1)
                        throw new Exceptions.PropertyNotFoundException(field, currType);
                    else
                    {
                        index++;
                        continue;
                        ...
                        if (newValue == null)
                        {
                            if (createWhenNeeded)
                            {
                                if (newType.IsClass)
                                {
                                    ...
                                    if (ci == null)
                                        throw new NotSupportedException(string.Format(CoreResources.NoConstructor, newType.Name));
                                    ...
                                    throw new NotSupportedException(string.Format(CoreResources.NotSupportedInterface, Constants.PROPERTY_SETTER));
                            }
                            else
                            {
                                destination = null;
                ...
                else
                {
                    ...
                    if (metas != null && metas.Length > 0)
                        meta = metas[0] as MetadataTypeAttribute;
